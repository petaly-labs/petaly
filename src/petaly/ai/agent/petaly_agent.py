        ...
        try:
            ...
            if isinstance(e, AttributeError):
                ...
                return (
                    ...
                    f"Example configuration in petaly.ini:\n\n"
                    f"\[ai_settings]\n"
                    f"# Select the LLM provider to use openai or anthropic. (default: openai)\n"
                    f"llm_provider={provider.lower()}\n"
                    f"# Select the LLM model to use. Tested with openai (gpt-4o), anthropic (claude-3-opus-20240229)\n"
                    f"llm_model=gpt-4\n"
                    f"# Path to the memory file. Default is ~/.petaly/agent_memory.json\n"
                    f"agent_memory_file=~/.petaly/agent_memory.json\n"
                    f"# Provide AI_AGENT_API_KEY as an environment variable: export AI_AGENT_API_KEY=\"YOUR-AI-AGENT-API-KEY-OR-TOKEN\"\n"
                    f"# Alternative and for the test purpose you can set it directly here\n"

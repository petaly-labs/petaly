...
class BQConnector():
    ...
    def execute_sql(self, query):
        ...
        try:
            ...
            logger.debug(f"New table {job.destination.project}.{job.destination.dataset_id}.{job.destination.table_id} is created.")
        except exceptions.GoogleCloudError as err:
            logger.debug(query)
            logger.error(err)
            pass
    ...
    def get_metadata_result(self, metadata_query):
        logger.debug(f"Execute meta query.")
        try:
            bq_client = bigquery.Client()
    ...
    def load_from(self, bq_job_config, data_fpath, table_id, load_from_bucket, region):
        ...
        logger.debug(f"Load data from the file {data_fpath}")
    ...
    def drop_table(self, table_id):
        ...
        """
        logger.debug(f"DROP table if exist {table_id}")
        try:
            bq_client = bigquery.Client()
            bq_client.delete_table(table_id, not_found_ok=True)  # Make an API request.
            logger.debug(f"Table {table_id} was dropped.")
        except exceptions.GoogleCloudError as err:
            logger.error(err)
            pass

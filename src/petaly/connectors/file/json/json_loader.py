...
class JsonLoader(FLoader):
    ...
    def load_from(self, loader_obj_conf):
        ...
        if dir_exists is True and files_in_dir> 0:
            ...
            # Filter out CSV files - only copy json files to target directory
            # CSV files remain in workspace for reference/debugging
            json_files = [f for f in file_list if f.endswith('.json')]
            
            if not json_files:
                logger.warning(f"No json files found in file_list. CSV files are kept in workspace and not copied to target directory.")
